...
Correct: retain `record.title` and `record.end - record.start`; attach only `record.schedule`. Use two fluid columns by default, three at panel width >=720px, and only one below 350px. Compact internal spacing below 520px keeps two columns usable at 360px. Browser assertions must check column count and full row coverage, not a fixed card width cap. The Beijing date block includes day, month, weekday and time. Room-owner colors style the card without implying participation. Render external text with textContent. Participant names belong in accessibility labels/tooltips, not visible adjacent labels.
