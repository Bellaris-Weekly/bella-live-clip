...
await build({entryPoints:['tests/browser.js'],outfile:'tests/artifacts/browser.js',bundle:true,format:'iife',target:'chrome110',loader:{'.html':'text','.css':'text'}});
const fixture=process.env.BILI_CLIP_FIXTURE || 'evidence/mediabunny-test.mp4';
const hlsFixture=process.env.BILI_CLIP_HLS_FIXTURE || 'tests/artifacts/preview-fragmented.mp4';
...
createServer(async(req,res)=>{
 ...
 if(req.method==='POST'&&path.startsWith('/artifact/')) {const chunks=[];for await(const chunk of req)chunks.push(chunk);await writeFile(resolve('tests/artifacts',basename(path)),Buffer.concat(chunks));res.end('ok');return;}
 ...
 const files={'/browser.js':'tests/artifacts/browser.js','/fixture.mp4':fixture,'/raw.m4s':hlsFixture,'/bella-live-clip.user.js':'bella-live-clip.user.js'};
