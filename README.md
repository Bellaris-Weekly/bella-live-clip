...
默认快捷键为 **Alt＋Shift＋C**。点击入口或使用快捷键均可收起面板。拖动成员栏或剪辑页导航栏的空白处可移动窗口；入口可拖动，窗口八个方向可缩放并记住位置。重置位置位于油猴菜单的“重置窗口位置”。
...
- **播放控制：** 左侧设为开始，中间为播放／暂停图标与当前时间，右侧设为结束。拖动播放位置或选区边界后恢复拖动前的播放状态。
