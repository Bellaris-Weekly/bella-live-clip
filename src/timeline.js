...
export function createTimeline({track,startHandle,endHandle,selectionElement,playhead,ticks,labels,onPreview,onScrubStart,onScrubEnd,onSelection=()=>{}}) {
 ...
 return {reset(duration,next={start:0,end:duration}){total=duration;view={start:0,end:total};setSelection(next);},setSelection,getSelection:()=>({...selection}),getView:()=>({...view}),setCurrent(t){if(!drag){current=t;render();}},lock(value){locked=value;render();},refit};
}
