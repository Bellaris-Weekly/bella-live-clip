...
import {MEMBERS,DEFAULT_SHORTCUT,clamp,parseTime,formatTime,formatDate,formatBytes,roomIdFromUrl,normalizeShortcut,formatShortcut,matchesShortcut,isEditing,constrainRect,resizeRect,fileName} from './core.js';
...
export function createApp({api,get=(_,fallback)=>fallback,set=()=>{},pageUrl=location.href}){
 ...
 const timeline=createTimeline({track:$('timeline'),startHandle:$('startHandle'),endHandle:$('endHandle'),selectionElement:$('selection'),playhead:$('playhead'),ticks:$('ticks'),labels:$('timelineLabels'),pan:$('pan'),zoomIn:$('zoomIn'),zoomOut:$('zoomOut'),showAll:$('showAll'),focus:$('focusRange'),onPreview:t=>player.seek(t),onScrubStart:()=>{playingSelection=false;player.pause();},onChange:s=>{$('start').value=formatTime(s.start,true);$('end').value=formatTime(s.end,true);}});
 const player=createPlayer({video,loading:$('videoLoading'),api,status,onTime:t=>{timeline.setCurrent(t);$('clock').textContent=formatTime(t,true);if(playingSelection&&t>=timeline.getSelection().end){video.pause();playingSelection=false;}}});
 function controls(){root.querySelectorAll('#body button,#body input,#body select').forEach(el=>{el.disabled=busy;});timeline.lock(busy||!ready);$('download').hidden=page!=='edit';$('download').disabled=busy||!ready;$('cancel').hidden=!busy;$('progress').hidden=!busy;$('launcher').dataset.busy=busy;}
 ...
 async function library(refresh=false){player.clear();ready=false;showPage('library');renderCards();if(!refresh&&cache.has(member.id)){status('选择想剪辑的那场直播。');return;}await job(async signal=>{status('正在获取直播场次…');cache.set(member.id,await api.history(member,signal));renderCards();status('选择想剪辑的那场直播。');});}
 async function loadRecord(next,signal){
  record=next;ready=false;clearDownloads();showPage('edit');$('recordTitle').textContent=record.title;$('recordMeta').textContent=`${record.member} · ${formatDate(record.start)}${record.live?' · 本场直播':' · 历史回放'}`;
 ...
 const close=()=>{$('panel').hidden=true;player.pause();};
 ...
 const updateInputs=()=>{try{timeline.setSelection({start:parseTime($('start').value),end:parseTime($('end').value)},true);}catch(e){status(e.message,true);}};$('start').onchange=$('end').onchange=updateInputs;
 ...
 $('playSelection').onclick=()=>{player.seek(timeline.getSelection().start);playingSelection=true;video.play().catch(e=>status(e.message,true));};
